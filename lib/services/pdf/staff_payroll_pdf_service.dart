// lib/services/pdf/staff_payroll_pdf_service.dart
//
// Generates a one-page-per-month Staff salary register PDF, styled to
// match the app's brown Staff theme.
//
...
//
// Requires the `pdf` package — already added to pubspec.yaml for the
// Labour module (pdf + printing).
...
class StaffPayrollPdfService {
  Future<Uint8List> generateMonthlyPayrollPdf({
    ...
    required List<StaffSalaryHistoryModel> records,
  }) async {
    ...
    records.fold<double>(0, (s, r) => s + r.effectiveSalary);
    final totalPf = records.fold<double>(0, (s, r) => s + r.pfAmount);
    final totalEsi = records.fold<double>(0, (s, r) => s + r.esiAmount);
    final totalLic = records.fold<double>(0, (s, r) => s + r.lic);
    final totalMess = records.fold<double>(0, (s, r) => s + r.mess);
    ...
    final headerRow = pw.TableRow(
      decoration: const pw.BoxDecoration(color: PdfColor.fromInt(0xFF6D4C41)),
      children: headers
    ...
    final dataRows = records
      ...
      cell(r.effectiveSalary.toStringAsFixed(0)),
      cell(r.pfAmount.toStringAsFixed(0)),
      cell(r.esiAmount.toStringAsFixed(2)),
      cell(r.lic.toStringAsFixed(0)),
      cell(r.mess.toStringAsFixed(0)),
    ...
    final totalRow = pw.TableRow(
      ...
      children: [
        ...
        cell('', bold: true, fontSize: 11),
        cell(totalEffective.toStringAsFixed(0), bold: true, fontSize: 11),
        cell(totalPf.toStringAsFixed(0), bold: true, fontSize: 11),
        cell(totalEsi.toStringAsFixed(2), bold: true, fontSize: 11),
        cell(totalLic.toStringAsFixed(0), bold: true, fontSize: 11),
        cell(totalMess.toStringAsFixed(0), bold: true, fontSize: 11),
