...
//   PF            = 12% of (Basic + DA)   [only if record.pfEnabled]
//   ESI           = 0.75% of HRA          [only if record.esiEnabled]
//   Insurance     = flat ₹ (from MD record)
//   Welfare       = flat ₹ (from MD record)
//   TDS           = flat ₹ (from MD record)
//   Net Salary    = Gross Wages − (PF + ESI + Insurance + Welfare + TDS)
//
...
import 'md_management_screen.dart' show MDRecord, Shift, ShiftX;
...
class _MDCalculatorScreenState extends State<MDCalculatorScreen> {
  ...
  Future<void> _searchRecords(String text) async {
    try {
      ...
      if (!mounted) return;
      setState(() {
        _suggestions = snap.docs.map((d) => MDRecord.fromDoc(d)).toList();
      });
    } catch (_) {
  ...
  Future<void> _fetchRecord() async {
    ...
    try {
      final snap =
      await _mdRef.where('MDId', isEqualTo: id).limit(1).get();
      if (snap.docs.isEmpty) {
        ...
        setState(() {
          _record = MDRecord.fromDoc(snap.docs.first);
          _recordError = null;
        });
      }
    } catch (e) {
  ...
  void _calculate() {
    ...
    final pf = record.pfEnabled ? basicDa * 0.12 : 0.0;
    final esi = record.esiEnabled ? hra * 0.0075 : 0.0;
    final insurance = record.insurance;
    final welfare = record.welfare;
    final tds = record.tds;
    final totalDeductions = pf + esi + insurance + welfare + tds;
  ...
  Color _shiftDotColor(Shift shift) {
    switch (shift) {
      case Shift.morning:
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'Calculate MD salary with present days, PF and ESI deductions.',
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          height: 8,
                          decoration: BoxDecoration(
                            color: _shiftDotColor(rec.shift),
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    height: 8,
                    decoration: BoxDecoration(
                      color: _shiftDotColor(_record!.shift),
                      shape: BoxShape.circle,
                    ),
                  ),
                  const SizedBox(width: 8),
                  Expanded(
                    child: Text(
                      '${_record!.name} · ${_record!.shift.label} · ₹${_record!.monthlySalary.toStringAsFixed(0)}/mo',
                      style: const TextStyle(
